...
class WeatherRepositoryShould {
  private val givenAFakeWeatherData =
  ...
  private val givenAWeatherLocationData = WeatherLocationData(message = "like", cod = "200", count = 13, list = listOf(givenAFakeWeatherData))

  private val givenAFakeForecastData =
      WeatherForecastData(
          listOf(Daily(99,
              295.75,
              1599242400,
  ...
  private val givenAUserLocation = UserLocationDomainModel(4.0, 3.0)
  ...
  fun `get weather data from api and map to domain model`() {
    ...
    verify(weatherApi, only()).getWeather(location.latitude.toString(), location.longitude.toString())
  ...
  @Test
  fun `get locations weather data from api and map to domain model`() {
    val location = "Sevilla"

    whenever(weatherApi.getWeatherLocations(location)).thenReturn(Single.just(givenAWeatherLocationData))

    val response = weatherRepository.getWeatherLocations(location).test()

    verify(weatherApi, only()).getWeatherLocations(location)
    verify(mapper).mapLocationsDataToDomainModel(givenAWeatherLocationData)
